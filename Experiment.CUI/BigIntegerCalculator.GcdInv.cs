...
#define ASSERT_GCD_CALCULATION // Gcd(Span<uint>, Span<uint>) の計算経過を検証する
...
{
    /// <remarks>
    /// <see href="https://github.com/dotnet/runtime/blob/main/src/libraries/System.Runtime.Numerics/src/System/Numerics/BigIntegerCalculator.GcdInv.cs"/>
    /// </remarks>
    ...
    [Diagnostics.CodeAnalysis.SuppressMessage("Style", "IDE0058:式の値が使用されていません", Justification = "<保留中>")]
    [Diagnostics.CodeAnalysis.SuppressMessage("Style", "IDE0018:インライン変数宣言", Justification = "<保留中>")]
...
#if ASSERT_GCD_CALCULATION
            var expected = BigInteger.GreatestCommonDivisor(ToBigInteger(left), ToBigInteger(right));
#endif

            while (right.Length > 2)
            {
#if ASSERT_GCD_CALCULATION
                AssertGcd(left, right, expected);
#endif
                ulong x, y;
